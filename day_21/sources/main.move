...
module challenge::day_21 {
    ...
    fun test_create_farm(){
        let mut scenario=test_scenario::begin(@0x1);{ //start scenario and identify who made first transaction
            create_farm(test_scenario::ctx(&mut scenario)); //create farm in first transaction
        };
        test_scenario::next_tx(&mut scenario,@0x1);{ //move to next transaction
            let farm=test_scenario::take_shared<Farm>(&scenario); //take shared farm object for testing
