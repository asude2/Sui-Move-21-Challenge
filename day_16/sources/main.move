...
module challenge::day_16 {
    ...
    public struct Farm has key { //not a data structure, it is an object that can be owned
        id: UID, //(Unique Identifier) 
        counters: FarmCounters,
    }

    // TODO: Write a constructor 'new_farm' that:
    fun new_farm(ctx: &mut TxContext): Farm { //a function that creates a new object is defined as private.
        Farm{
            id:object::new(ctx),  //creates a new UID for the farm object
    ...
    fun test_farm(){
        ...
        assert!(farm.counters.planted==0,1);
        assert!(farm.counters.harvested==0,2);
        assert!(vector::is_empty(&farm.counters.plots),3);

        //!Clean up the farm object after the test.Because farm object has not drop ability.
